---
name: design-system
description: Kinetic Editorial design tokens and component patterns for the Nerofit fitness app. Load whenever building, styling, or reviewing any UI — screens, components, or visual changes. Ensures every screen matches the true-black + chartreuse editorial look.
---
...
- Use the presets in `src/theme/typography.ts` (`display`, `h1`, `h2`, `body`,
  `bodyMuted`, `labelCaps`) instead of inlining fontFamily/size/color.
...
Verify primitives visually on the Profile tab gallery (Phase 1 only — gets
replaced by the real Profile screen in Phase 6).

## Checklist before finishing a UI task
- [ ] Colors / spacing / radii come from theme tokens, not literals.
- [ ] Accent discipline respected.
- [ ] Headings use Hanken Grotesk; body uses Inter (via `typography` presets).
- [ ] Loading / empty / error states present for data screens.
- [ ] Text uses i18n keys (UZ / RU / EN), not hardcoded strings (Phase 2+).
