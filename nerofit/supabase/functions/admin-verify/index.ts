...
async function handlePost(req: Request): Promise<Response> {
  ...
  if (action === "dashboard_stats") {
    // All KPIs computed DB-side in one indexed round-trip (migration 0023):
    // no raw-row download, so PostgREST's 1000-row cap can't skew the counts.
    // The RPC returns the exact same json shape the panel already renders.
