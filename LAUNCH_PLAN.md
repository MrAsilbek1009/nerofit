...
| 16 | 🥗 Nutrition — ikkinchi ustun (`docs/nutrition-pillar-plan.md`) | 🟡 | Claude — **N1 ✅** regional food DB + crowdsource + Verified (migration `0019-mirolim_foods` + 41 taom seed; qidiruv lokal-avval; admin food-panel `docs/food-admin` + `admin-foods` edge). **N2 ✅** (1) natija-muharrir: **gram slider + porsiya/gram birlik** (OFF/lokal serving gramlari, AI `total_g`) + **"Fix with AI"** (matnli tuzatish → qayta baholash; edge deployed ✅); (2) **tez log**: meal-picker → Log-food hub (Tezkor/Katalog/Taomlarim) — recent + favoritlar (yulduzcha) + quick-add kkal + **meal builder** (`user_meals`, Mobbin/Cal AI referens). 🧑 handoff: `0020-mirolim_fast_log.sql` apply (favorite_foods+user_meals); 0019 migration+seed, `admin-foods` deploy. Qolgan (N3+): adaptiv TDEE, daily breakdown, streak modal ⬜ |
