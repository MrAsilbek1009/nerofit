...
| 16 | 🥗 Nutrition — ikkinchi ustun (`docs/nutrition-pillar-plan.md`) | 🟡 | Claude — **N1 ✅** regional food DB + crowdsource + Verified (migration `0019-mirolim_foods` + 41 taom seed; qidiruv lokal-avval; admin food-panel `docs/food-admin` + `admin-foods` edge). **N2 🟡** natija-muharrir: **gram slider + porsiya/gram birlik ✅** (OFF/lokal serving gramlari, AI `total_g`) + **"Fix with AI" ✅** (matnli tuzatish → qayta baholash, rasmli/rasmsiz). 🧑 handoff: 0019 migration+seed apply, `admin-foods`+`food-analysis` deploy. Qolgan: favoritlar/tez log, meal builder ⬜ |
