...
| 16 | 🥗 Nutrition — ikkinchi ustun (`docs/nutrition-pillar-plan.md`) | 🟡 | Claude — **N1 ✅** (PR #31) regional food DB + crowdsource + Verified (migration `0019-mirolim_foods` + 41 taom seed; qidiruv lokal-avval; admin food-panel `docs/food-admin` + `admin-foods` edge). **N2 ✅** (PR #35) (1) natija-muharrir: **gram slider + porsiya/gram birlik** (OFF/lokal serving gramlari, AI `total_g`) + **"Fix with AI"** (matnli tuzatish → qayta baholash; edge deployed ✅); (2) **tez log**: meal-picker → Log-food hub (Tezkor/Katalog/Taomlarim) — recent + favoritlar (yulduzcha) + quick-add kkal + **meal builder** (`user_meals`, Mobbin/Cal AI referens). **N3 ✅** (PR #36) adaptiv TDEE: EWMA vazn-trend + haftalik client-trigger qayta-hisob (data-gate bilan, adherence-neutral), `nutrition_targets` tarix (migration `0021-mirolim`), "Rejang yangilandi" kartasi. **N4 ✅** nutrition hub: Daily Breakdown ekrani (ring+makro+hafta grafigi) + Edit Goals (manual → adaptiv hurmat qiladi) + streak celebration modal (Cal AI uslubi, kuniga 1x). **N4.1 ✅** 6 derived badge (Profile "Yutuqlar", jadvalsiz/migratsiyasiz) + haftalik log-maqsad (5 kun) + health-score Breakdown'da. 🧑 handoff: `0020`+`0021` apply ✅ (2026-07-12). Qolgan: N5 freemium ⬜ |
...
| Food-scan kengaytmalari | 🤖 | **barcode skan + ingredient qidiruv ✅** (OpenFoodFacts) · **scan tarixi ✅** (qayta-log + o'chirish) · **rasm → Storage ✅** (client: `food-photos`ga upload + signed-url thumbnaillar; progressiv/best-effort). 🧑 handoff: `food-photos` bucket + Storage RLS (`0016-mirolim`) + `food-analysis` edge redeploy. · **"Fix with AI" ✅** (Phase 16 N2'da). Qolgan: favoritlar ⬜ |
